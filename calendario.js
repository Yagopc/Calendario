...
function renderCalendar() {
  ...
  for (let d=1; d<=ultimoDia.getDate(); d++) {
    ...
    // Busca esta parte en la función renderCalendar()
if (esHoy(fecha)) {
  cell.classList.add("today");
  // Asegúrate de que no hay otras clases que puedan interferir
  cell.style.animation = "blink 1.5s infinite"; // Esto es opcional, ya que CSS lo maneja
}
...
// ... Tu código anterior para el calendario: creación de celdas, eventos, etc ...

// Coordenadas Jaén capital
...
async function fetchMeteoDataJaen() {
  ...
  try {
    ...
    if (data.hourly) {
      const now = new Date();
      // Convertir a hora local Madrid (la API ya lo devuelve bien por timezone)
...
// Añade estas funciones al final del archivo, antes de cargarDatos();

async function actualizarDatosAstronomicos() {
  try {
    // Coordenadas de Jaén capital
    ...
    // Puedes mostrar valores por defecto o manejar el error de otra manera
  }
}

function calcularFaseLuna(fecha) {
  // Algoritmo aproximado para calcular la fase lunar
  // Basado en ciclos lunares (29.53 días)
  
  // Fecha de referencia de luna nueva (puede ajustarse)
  const referenciaLunaNueva = new Date('2000-01-06');
  
  // Diferencia en días
  const diffDias = (fecha - referenciaLunaNueva) / (1000 * 60 * 60 * 24);
  
  // Ciclo lunar actual (0-29.53)
  const cicloLunar = diffDias % 29.530588853;
  
  // Determinar fase y porcentaje
  let fase, porcentaje;
